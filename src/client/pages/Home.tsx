        ...
        <div className="grid-2">
          <div>
            ...
            <ol className="how">
              ...
              <li><b>{t('The author’s agent drafts; the author approves.', '저자의 에이전트가 초안을 쓰고, 저자가 승인합니다.')}</b> {t('In the studio, an agent reads where the article is thin and proposes level variants, plainer rewrites and FAQ entries as tool calls. Nothing is published without the author’s click.', '스튜디오에서 에이전트가 얇은 부분을 읽고 수준별 판·쉬운 버전·FAQ를 툴 호출로 제안합니다. 저자의 클릭 없이는 아무것도 게시되지 않습니다.')} <a href="/studio/compound-interest" onClick={(e) => { e.preventDefault(); navigate('/studio/compound-interest'); }}>{t('Open the studio →', '스튜디오 열기 →')}</a></li>
