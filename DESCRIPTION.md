...
## How we built it
- **WebMCP**: `document.modelContext.registerTool()` with `AbortSignal` surfaces (home: 5 tools; article: 17), surface names that encode the edition (`article:webmcp:expert:ko`), `readOnlyHint` on reads, loose schemas with strict validation and self-correcting errors, `next_step` in every result.
