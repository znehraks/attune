...
card('01-title', 'docs/video/build/card-title.png', S['01-title'].audio + 1.2);
const order = ['02-problem', '03-declare', '04-korean', '05-friction', '06-interactive', '07-author', '08-publisher'];
for (let i = 0; i < order.length; i++) { const id = order[i]; const start = marks[id]; const end = i + 1 < order.length ? marks[order[i + 1]] : marks['08-end']; clip(id, videoA, start, end - start); }
card('09-how', 'docs/video/build/card-how.png', S['09-how'].audio + 0.8);
card('10-end', 'docs/video/build/card-end.png', S['10-end'].audio + 2.0);
